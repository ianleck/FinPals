...
export const trips = pgTable('trips', {
  ...
  createdBy: text('created_by').references(() => users.telegramId).notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
...
export const expenses = pgTable('expenses', {
  ...
  paidBy: text('paid_by').references(() => users.telegramId).notNull(),
  createdBy: text('created_by').references(() => users.telegramId).notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
...
export const settlements = pgTable('settlements', {
  ...
  tripId: uuid('trip_id').references(() => trips.id),
  fromUser: text('from_user').references(() => users.telegramId).notNull(),
  toUser: text('to_user').references(() => users.telegramId).notNull(),
  amount: decimal('amount', { precision: 10, scale: 2 }).notNull(),
  currency: text('currency').default('USD'),
  createdBy: text('created_by').references(() => users.telegramId).notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
...
export const userPreferences = pgTable('user_preferences', {
  userId: text('user_id').references(() => users.telegramId).primaryKey(),
