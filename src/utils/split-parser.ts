 ...
 */
export function formatSplitInfo(splits: Map<string, SplitInfo>, totalAmount: number): string {
    const splitTypes = new Set<string>();
    let message = '';

    for (const [mention, split] of splits.entries()) {
        if (split.type === 'percentage') {
            message += `${mention}: ${split.value}% `;
            splitTypes.add('percentage');
        } else if (split.type === 'share') {
            message += `${mention}: ${split.value} shares `;
            splitTypes.add('share');
        } else {
            message += `${mention}: $${split.value} `;
            splitTypes.add('amount');
        }
    }
