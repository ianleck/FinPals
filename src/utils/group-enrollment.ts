...
export async function enrollAllGroupMembers(ctx: Context, db: Database, groupId: string): Promise<{ enrolled: number; failed: number }> {
	...
	try {
		...
		for (const admin of admins) {
			if (!admin.user.is_bot) {
				try {
					...
					logger.error(`Failed to enroll admin ${admin.user.id}`, error);
					failed++;
				}
			}
		}
		...
		logger.error('[Error] Failed to enroll group members', error);
		throw error;
	}
}
...
export async function getGroupEnrollmentStatus(
	ctx: Context,
	db: Database,
	groupId: string,
): Promise<{
